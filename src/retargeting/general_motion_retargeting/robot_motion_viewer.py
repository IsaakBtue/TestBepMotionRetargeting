...
def copy_mujoco_visual(src: Any, dst: Any) -> None:
    """Copy visualization parameters from a reference model (e.g. T1 MJCF) onto another model."""

    def walk(s_obj, d_obj, depth: int = 0) -> None:
        if depth > 16 or s_obj is None or d_obj is None:
            return
        for name in dir(s_obj):
            if name.startswith("_"):
                continue
            try:
                sv = getattr(s_obj, name)
            except Exception:
                continue
            if callable(sv):
                continue
            try:
                dv = getattr(d_obj, name)
            except Exception:
                continue
            if isinstance(sv, np.ndarray):
                if isinstance(dv, np.ndarray) and sv.shape == dv.shape and sv.dtype == dv.dtype:
                    dv[:] = sv
            elif isinstance(sv, (bool, int, float, np.integer, np.floating)):
                if isinstance(dv, (bool, int, float, np.integer, np.floating)):
                    setattr(d_obj, name, type(dv)(sv))
            elif hasattr(sv, "__dir__") and not isinstance(sv, (str, bytes)):
                walk(sv, dv, depth + 1)
            else:
                pass
...
class RobotMotionViewer:
    def __init__(self,
                ...
                model_xml_path=None,
                visual_style_reference_xml=None,
                ):
        
        self.robot_type = robot_type
        if model_xml_path is not None:
            self.xml_path = pathlib.Path(model_xml_path)
        else:
            self.xml_path = ROBOT_XML_DICT[robot_type]
        self.model = mj.MjModel.from_xml_path(str(self.xml_path))
        if visual_style_reference_xml is not None:
            ref_path = pathlib.Path(visual_style_reference_xml)
            ref = mj.MjModel.from_xml_path(str(ref_path))
            copy_mujoco_visual(ref.vis, self.model.vis)
